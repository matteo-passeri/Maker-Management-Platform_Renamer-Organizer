...
for folder in os.listdir(FOLDERS_TO_FIX):
    ...
    if os.path.isdir(folder_path):
        ...
        new_folder = folder
        # If there are at least 5 digits at the end, remove them (remove Id).
        if new_folder[-1].isdigit() & new_folder[-5].isdigit():
            temp_string = ""
            for c in new_folder[::-1]:
                if c.isdigit():
                    temp_string = c + temp_string
            print(temp_string)
            print(len(temp_string))
            # Double check that are at least 5 digits in a row.
            if len(temp_string) > 4:
                new_folder = new_folder.replace(temp_string, '')
                # If the folder has digits at the end, probably has ' - ' too, remove it.
